...
def solve_tsp_numpy( distances, optim_steps=3 ):
    ...
    is_sparse = isinstance(distances, lil_matrix) #if scipy sparse matrix is used
    N = len(distances) if not is_sparse else distances.shape[0]
    if is_sparse:
    ...
    def join_segments():
        ...
        def pairs_by_dist_np():
            ...
            for i in xrange(N):
                row_size = N-i-1
                if is_sparse:
                    pairs[idx:(idx+row_size)] = [ (distances[i,j], i, j)
                                                 for j in xrange(i+1, N) ]
                else:
                    dist_i = distances[i]
